...
```{r setup}
# List of packages that we need ############################################################# PUT INTO ALPHABETICAL ORDER
packages <- c(
    "ggplot2", "knitr", "mia", "dplyr", "miaViz", "MOFA2",  "patchwork",
    "reticulate", "MGnifyR", "IntegratedLearner", "SuperLearner", "bartMachine",
    "mcmcplots", "tidyverse", "shadowtext", "ComplexHeatmap", "stringr",
    "cowplot", "caret", "scater", "ggsignif", "ggpubr"
    )
...
data cleaning. For instance, we need to handle missing data, transform data assays,
...
4. Transform data with logarithmic methods to prepare it for statistical analysis.
...
for compositional metagenomic data
...
Below, we see an upset plot that illustrates the overlap between different
experiments and their corresponding samples.
...
We need to add a few columns for the subsequent data analysis steps. We are
interested in investigating potential differences between treatment
and control. Furthermore, we will only focus on salmons at day 60 because
these salmons were sampled after experimental period. In contrast, day-0 salmons
that were sampled immediately in the beginning of the trial.
...
# Add treatment group to btoth experiment's sample metadata
colData(mae[[1]])[["treatment_group"]] <- colData(mae)[mae[[1]][["animal"]], "treatment_group"]
colData(mae[[2]])[["treatment_group"]] <- colData(mae)[mae[[2]][["animal"]], "treatment_group"]
...
mass_median <- median(as.numeric(colData(mae)[, "host.gutted.mass"]), na.rm = TRUE)
colData(mae)[["mass_category"]] <- ifelse(
  as.numeric(colData(mae)[["host.gutted.mass"]]) > mass_median, "heavy", "light"
)
```
...
prevalence threshold to use. We use 0.01% detection level to filter out
extremely low-abundant species present in each sample.

```{r prevalence_histgoram}
...
```{r add_treatment_group1}
# Compare two sets of animal samples in order to add the treatment concentration
# column to metagenomic data
setdiff(colData(mae)$animal, colData(mae[[2]])$animal) # No difference

# We then can simply add treatment_concentration column to metagenomic data
colData(mae[[2]])$treatment_concentration <-
  colData(mae)$`Treatment concentration`

# Add treatment_group (control vs. treatment) to metagenomic data
metagenomic_data <- colData(mae[[2]])
metagenomic_data$treatment_group <-
  ifelse(metagenomic_data$treatment_concentration == 0, "control", "treatment")
colData(mae[[2]]) <- metagenomic_data
```

With a reasonable prevalence of 20%, and relative abundance detection threshold
of 0.2%, only bacterial genera remain.

```{r}
# Agglomerate by prevalence by genus
altExp(mae[[2]], "prev_genus") <- agglomerateByPrevalence(mae[[2]],
    assay.type = "relabundance",
    rank = "Genus",
    na.rm = TRUE,
    agg.na.rm = TRUE,
...
From fatty acid data, we select those features that varies.
To decide the threshold of filtering, we can use a histogram of standard
deviations

```{r standard_deviation_hist}
...
rowData(mae[[1]])[["sd"]] <- apply(assay(mae[[1]], "log10"), 1, sd)
# Histogram of standard deviations
p <- ggplot(rowData(mae[[1]]), aes(x = sd)) +
  geom_histogram(bins = 7) +
  labs(title = "Standard deviations of log10-transformed fatty acid concentrations")
p
```

A good threshold would be at 0.1. Thus, we can filter out any fatty acid whose
standard deviation of concentration is below 0.1.

```{r filter_low_sd}
altExp(mae[[1]], "not_sd_0") <- mae[[1]][rowData(mae[[1]])[["sd"]] > 0.01, ]
```
...
We log-transform fatty acid assay.
...
# Wait until bacth transformation PR is ready in mia
altExp(mae[[1]], "not_sd_0") <- transformAssay(
    altExp(mae[[1]], "not_sd_0"),
    assay.type = "counts",
    MARGIN = "samples",
    method = "log10",
    pseudocount = TRUE
...
mae[[2]] <- transformAssay(mae[[2]], method = "relabundance", MARGIN = "samples")
mae[[2]] <- transformAssay(
    mae[[2]],
    assay.type = "relabundance",
    method = "clr",
    pseudocount = TRUE, MARGIN = "samples"
  )
mae[[2]] <- transformAssay(
    mae[[2]],
    assay.type = "clr",
...
Before proceeding to data analysis, we will employ some basic data exploration
methods:

1. Abundance plot of metagenomic species
2. Alpha-diversity
...
```{r plot_abundance}
plotAbundanceDensity(altExp(mae[[2]], "prev_genus"), assay.type = "clr", colour_by = "treatment_group") +
...
mae[[2]] <- addAlpha(mae[[2]], assay.type = "counts",  index = c("observed", "shannon"))

plotColData(mae[[2]], "observed", "treatment_group", show_violin  = FALSE, show_box = TRUE)
```
...
# Calculate p values
pvals <- pairwise.wilcox.test(mae[[2]][[index]], mae[[2]][[group_var]], p.adjust.method = "fdr")
...
draw(p, padding = unit(c(2, 20, 2, 25), "mm"))
```
...
p <- plotReducedDim(mae[[2]], "MDS_bray", colour_by = "treatment_group")

# Calculate explained variance
e <- attr(reducedDim(mae[[2]], "MDS_bray"), "eig")
rel_eig <- e / sum(e[e > 0])
...
# column
colData(mae_temp) <- colData(mae_temp)[ , c("mass_category"), drop = FALSE]

# Extract only transformed metagenomic assays for MOFA+ analysis

assays(mae_temp[[1]]) <- assays(mae_temp[[1]])[names(assays(mae_temp[[1]])) %in% c("standardize")]
assays(mae_temp[[2]]) <- assays(mae_temp[[2]])[names(assays(mae_temp[[2]])) %in% c("standardize")]
...
```{r var_factor1}
plot_variance_explained(model, x = "group", y = "view", factor = 1, legend = TRUE)
```
...
```{r var_factor2}
plot_variance_explained(model, x = "group", y = "view", factor = 2, legend = TRUE)
```
...
```{r var_factor3}
plot_variance_explained(model, x = "group", y = "view", factor = 3, legend = TRUE)
```
...
Random Forest algorithm is used for classification. We also attempt to classify
...
Furthermore, we apply it to investigate whether we can reliable classify salmons
into "heavy" (above mass median) or "light" (below or equal to mass) salmons.
We hypothesize that "heavy" salmons are distinct from "light" salmons that
might be due to differences in treatment, genomics, or starting weight baseline.
...
metabolomics_df <- mae[["fatty_acids_mg"]] |>
  assay("log10") |>
  as.data.frame()
metagenomics_df <- mae[["metagenomic_amplicon"]] |>
  assay("clr") |>
  as.data.frame()
sample_meta_df <- colData(mae) |>
  as.data.frame()

# Replace "sample" names to animal accession numbers
names(metabolomics_df) <- colData(mae[["fatty_acids_mg"]])[, "animal"]
names(metagenomics_df) <- colData(mae[["metagenomic_amplicon"]])[, "animal"]
...
all(colnames(metagenomics_df) == rownames(colData(mae))) # TRUE
all(colnames(metabolomics_df) == rownames(colData(mae))) # TRUE
```
...
```{r prepare_integrated_learner_functions}
create_sample_metadata <- function(df, Y, subjectID, factor) {
  if (factor == TRUE) {
    Y <- as.numeric(factor(df[[Y]]))
  ...
  rownames(sample_metadata) <- subjectID
...
```{r}
sample_metadata <- create_sample_metadata(df = sample_meta_df,
                                          Y = "mass_category",
...
We apply Random Forest as our base model for per-layer predictions, and
non-negative least squares method to make final predictions.
...
# Control: 0
# Treatment: 1
sample_metadata <- create_sample_metadata(df = mae_day_60_coldata,
                                          Y = "treatment_group",
                                          subjectID = colnames(metabolomics_df),
                                          factor = TRUE)
...
sample_metadata_gutted_weight <- create_sample_metadata(df = mae_day_60_coldata,
                                                        Y = "host.gutted.mass",
                                                        subjectID = colnames(metabolomics_df),
                                                        factor = FALSE)
...
With BART machines, we can also perform classification.
...
bart_fit <- IntegratedLearner:::IntegratedLearner(feature_table = feature_table_train,
                               sample_metadata = sample_metadata_train,
                               feature_metadata = feature_metadata,
                               feature_table_valid = feature_table_valid,
                               sample_metadata_valid = sample_metadata_valid,
