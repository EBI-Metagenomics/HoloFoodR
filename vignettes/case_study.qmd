...
colData(mae_sub) <- colData(mae_sub)[ , c("treatment_code"), drop = FALSE]
...
the diametrically opposed effects  effects in factors 1 and 3 compared to factor 2. 
Specifically, it has the largest effects
in factors 2 and 3, in the former case the effect is positive, where in the latter
case it is negative. Conversely, it has second most significant negative effect on factor 1.
