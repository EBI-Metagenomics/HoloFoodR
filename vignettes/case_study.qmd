...
3. Transform data with logarithmic methods to prepare it for statistical analysis.
...
4. Agglomerate metagenomic data by prevalence. We will keep only the species
...
mae_sub[[2]] <- transformAssay(
    mae_sub[[2]], method = "relabundance")
mae_sub[[2]] <- transformAssay(
    mae_sub[[2]],
...
altExp(mae_sub[[2]], "prev_genus") <- agglomerateByPrevalence(mae_sub[[2]],
    assay.type = "relabundance",
    rank = "Genus",
    na.rm = TRUE,
    agg.na.rm = TRUE,
...
We now can extract only log10-transformed metabolomic assays because raw
concentrations are no longer needed.
...
# Extract metagenomic data
tse <- mae[[2]]

# Add treatment vs. control into TSE object at day 60
day60 <- colData(tse)$trial.timepoint == 60
tse <- tse[, day60]
colData(tse)$treatment_group <- ifelse(colData(tse)$host.diet.treatment.concentration == 0,
                                       "control", "treatment")
```
...
comb <- split(t(combn(levels(df$treatment_group), 2)), 
           seq(nrow(t(combn(levels(df$treatment_group), 2)))))

ggplot(df, aes(x = treatment_group, y = shannon)) +
...
In the following workflow steps, we will only use salmons at day 60 because, they
might be differ from each other while salmons at day 0 are more similar to each
other despite being categorized into "control" and "treatment" groups, although
they did not have any treatment.

```{r fetch_day60_salmons}
# Substitute original metagenomic TreeSummarizedExperiment with agglomerated
# version
...
tse_pcoa <- runMDS(mae_day_60[["metagenomic_amplicon"]],
                   FUN = vegan::vegdist,
                   methods = "bray",
                   assay.type = "relabundance",
                   name = "MDS_bray")
...
Before cross-association analysis, we can filter out all features with low
standard deviation to exclude potential false positives from cross-association
analysis. To decide the threshold of filtering, we can use a histogram of standard
deviations

```{r standard_deviation_hist}
sd <- as.data.frame(apply(assay(mae_sub[[1]], "log10"), 1, sd))
colnames(sd) <- c("standard_deviation")

# Histogram of standard deviations
p <- ggplot(sd, aes(x = standard_deviation)) +
...
```{r filter_low_sd}
tse_filter <- mae_sub[[1]][apply(assay(mae_sub[[1]], "log10"), 1, sd) > 0.1, ]
altExp(mae_sub[[1]], "not_sd_0") <- tse_filter
```

Now, we can perform cross-association analysis using Kendall method and display
significantly correlated species and fatty acids.
