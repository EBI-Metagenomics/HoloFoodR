...
mae_sub[[2]] <- transformAssay(
    mae_sub[[2]], assay.type = "relabundance", method = "clr", pseudocount = TRUE)
```
...
compute_prev("Aliivibrio")
```


```{r agglomerate_prevalence}
# For cross-correlation analysis and MOFA2, replace original metagenomic data
# with agglomerated one
# Agglomerate by prevalence and rank to remove low-abundant species
...
# Compute number of reads and sort in descending order
assay(agglomerated_genus, "counts") |> rowSums() |> sort(decreasing = T)
```


Sort reads for the top 10 species.

```{r}
top_prevalent_genera <- sort(getPrevalence(mae_sub[[2]],
    rank = "Genus",
    na.rm = TRUE,
    onRankOnly = TRUE
  ), decreasing = T) |> names() |> head()

for (genus in top_prevalent_genera) {
  print(genus)
  print(sort(assay(agglomerated_genus, "counts")[genus, ], decreasing = T))
}


print(sort(assay(agglomerated_genus, "relabundance")["Ammopiptanthus", ], decreasing = T))
```
...
colData(mae_day_60) <- colData(mae_day_60)[ , c("treatment_code", treatment_group, "host.gutted.mass", "mass_category"), drop = FALSE]
...
```
Control and treatment groups are mixed which again suggests that there is no
much difference between these two groups in both heavy and light salmons.
...
custom_plotter("metagenomic_assembly", factor = 3)
```
...
plot_weights(
    ...
    labs(title = "Top 10 weights for the first factor in fatty acid view") +
...
plot_weights(
    ...
    labs(title = "Top 10 weights for the second factor in metagenomic view") +
...
plot_weights(
    ...
    labs(title = "Top 10 weights for the third factor in metagenomic view") +
  theme(plot.title = element_text(size = 10),
        axis.title.x = element_text(size = 15),
        axis.title.y = element_text(size = 15)
  )
...
plot_factors(model, factors = c(1, 2, 3), color_by = "mass_category")
```


No separation between heave and light salmons.
...
plot_dimred(
  ...
  labs(title = "UMAP plot of MOFA model by mass category")
```
...
names(metabolomics_df) <- colData(mae_sub[[1]])[, "animal"]
names(metagenomics_df) <- colData(mae_sub[[2]])[, "animal"]
