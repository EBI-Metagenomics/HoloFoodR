...
mae <- mae[, , c("FATTY ACIDS MG", "metagenomic_amplicon")]
```
...
agglomerateByPrevalence(mae_sub[[2]],
                        rank = "Genus",
                        assay.type = "relabundance",
                        na.rm = TRUE,
                        detection = 0.001)
...
agglomerated_genus <- agglomerateByPrevalence(mae_sub[[2]],
    assay.type = "relabundance",
    rank = "Genus",
    na.rm = TRUE,
    onRankOnly = T
...
top_prevalent_genera <- sort(getPrevalence(mae_sub[[2]],
    ...
    na.rm = TRUE,
    detection = 0.001,
  ), decreasing = T) |> names() |> head(20)


print(sort(assay(agglomerated_genus, "relabundance")["Aliivibrio", ], decreasing = T))
```
