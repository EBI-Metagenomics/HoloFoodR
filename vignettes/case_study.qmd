...
# Agglomerate microbiota assays by rank
for (i in indices) {
  altExp(tse[[i]], rank) <- agglomerateByRank(tse[[i]], rank, na.rm = TRUE)

  altExp(tse[[i]], rank) <- transformAssay(altExp(tse[[i]], rank), MARGIN = "samples",
                      method = "relabundance")

  altExp(tse[[i]], rank) <- transformAssay(altExp(tse[[i]], rank), MARGIN = "samples",
                      method = "clr", pseudocount = 0.0003)

  plotAbundanceDensity(altExp(tse[[i]], rank) , layout = "jitter",
                     assay.type = "clr",
...
altExp(tse[[6]], rank) <- agglomerateByRank(tse[[6]], rank, na.rm = TRUE)

altExp(tse[[6]], rank) <- transformAssay(altExp(tse[[6]], rank), MARGIN = "samples",
                    method = "relabundance")

altExp(tse[[6]], rank) <- transformAssay(altExp(tse[[6]], rank), MARGIN = "samples",
                    method = "clr", pseudocount = 0.0003)

plotAbundanceDensity(altExp(tse[[6]], rank) , layout = "jitter",
                   assay.type = "clr",
                   n = 40, point_size=1, point_shape=19,
                   point_alpha=0.1,
                   )

altExp(tse[[5]], rank) <- agglomerateByRank(tse[[5]], rank, na.rm = TRUE)

altExp(tse[[5]], rank) <- transformAssay(altExp(tse[[5]], rank), MARGIN = "samples",
                    method = "relabundance")

altExp(tse[[5]], rank) <- transformAssay(altExp(tse[[5]], rank), MARGIN = "samples",
                    method = "clr", pseudocount = 0.0003)

plotAbundanceDensity(altExp(tse[[5]], rank) , layout = "jitter",
                   assay.type = "clr",
                   n = 40, point_size=1, point_shape=19,
                   point_alpha=0.1,
                   )
