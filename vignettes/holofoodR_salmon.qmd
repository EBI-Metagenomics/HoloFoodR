...
holofood_cache <- file.path("..", ".HoloFoodR_cache")
if (!dir.exists(holofood_cache)) {
  dir.create(holofood_cache, recursive = TRUE)
}
mgnify_cache <-  file.path("..", ".MGnifyR_cache")
if (!dir.exists(mgnify_cache)) {
  dir.create(mgnify_cache, recursive = TRUE)
}

# Get salmon samples
salmons <- HoloFoodR::doQuery("animals", system = "salmon", use.cache = TRUE, cache.dir = "../.HoloFoodR_cache/")
...
samples <- HoloFoodR::doQuery("samples", use.cache = TRUE, cache.dir = "../.HoloFoodR_cache", system = "salmon")
