...
# 
# sampleMap(salmon_mae)[sampleMap(salmon_mae)[, "assay"] == "metagenomic_assembly", "colname"]
# 
# sample_map <- sampleMap(salmon_mae)[sampleMap(salmon_mae)[, "assay"] == "metagenomic_assembly", "colname"]
# cd1 <- colData(salmon_metagenomic_tse)
# cd1 <- cd1[match(sample_map, rownames(cd1)), ]
# cd1 <- DataFrame(cd1)
# 
# colData(salmon_metagenomic_tse) <- cd1
# 
# salmon_metagenomic_tse
# 
# salmon_mae[[6]] <- salmon_metagenomic_tse
...
#   all = FALSE
# )
# salmon_metagenomic_tse_test <- salmon_metagenomic_tse
tse1 <- salmon_metagenomic_tse_test
...
# Match with MGnify data
# cd <- cd[match(rownames(colData(salmon_metagenomic_tse_test)), cd$sample_biosample), ]
# 
# cd <- DataFrame(cd)
# #colData(salmon_metagenomic_tse) <- NULL
# colData(tse1) <- cd
...
# Add trial timepoint to colData(salmon_mae)
# animal_meta <- as.data.frame(colData(salmon_mae))
# tse_meta <- as.data.frame(colData(tse1)) |>
#   select(c("animal", "trial.timepoint"))
# merged_coldata_meta <- merge(animal_meta, tse_meta, by = "animal")
# merged_coldata_meta <- DataFrame(merged_coldata_meta)
# rownames(merged_coldata_meta) <- merged_coldata_meta$animal
# colData(salmon_mae) <- merged_coldata_meta
